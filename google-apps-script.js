/*************************************************************************
 * HOUSEHOLD EXPENSE LEDGER & WEB APP API — COMPLETE APPS SCRIPT
 * For: Kashif Zia & Asif Zia (50/50 Shared Household Expenses)
 * Sheet: Home Expense 01022026
 *
 * INSTRUCTIONS:
 * 1. Open your Google Sheet -> Extensions -> Apps Script.
 * 2. In 'Code.gs', paste the Code.gs section below.
 * 3. In 'Webapp api.gs', paste the Webapp api.gs section below.
...
function setupSystem() {
  ...
  recalcDashboard();
  SpreadsheetApp.getUi().alert('Setup complete ✅\n\nDashboard and Settlement Ledger initialized.');
}
...
function computeBalances_() {
  ...
  return {
    ...
    // Dual compatibility with legacy and new client schemas
    openingBalance: {
  ...
  try {
    ...
    if (action === 'verifyPin') {
      ...
      if (expectedPin && userPin === expectedPin) {
        ...
        // If sheet cell B6 is not set yet, initialize it with this PIN
        if (!settings) {
          settings = ss.insertSheet(CONFIG.SETTINGS_SHEET_NAME);
        }
        settings.getRange('A6').setValue('Security PIN (Settings Cell B6)').setFontWeight('bold');
    ...
    if (expectedPin) {
      ...
      if (clientPin !== expectedPin) {
        return jsonResponse_({
          error: 'Unauthorized write operation. Incorrect or missing Security PIN (Settings Cell B6).'
        });
      }
    }
...
function getExpensesJson_() {
  ...
  var idCol = data.col.id > -1 ? data.col.id : 13;
...
function addExpense_(p) {
  ...
  // Find first genuinely empty row (Row 36)
  var targetRowIdx = -1;
  var lastSerial = 0;
  ...
  rowArr[0] = newSr;
  rowArr[1] = p.date ? new Date(p.date) : new Date();
  rowArr[2] = p.category || 'Miscellaneous';
  rowArr[3] = p.details || '';
  rowArr[4] = Number(p.amount) || 0;
  rowArr[5] = String(p.paidBy || '').toLowerCase().indexOf('kashif') >= 0 ? 'kashif' : 'asif';
  rowArr[6] = p.vendor || '';
  rowArr[7] = String(p.status || 'Paid').toLowerCase();
  rowArr[8] = half;
  rowArr[9] = half;
  rowArr[10] = '';
  rowArr[11] = Utilities.formatDate(new Date(), Session.getScriptTimeZone() || 'GMT+5', 'dd-MM-yyyy HH:mm');
  rowArr[12] = 'Added via Web App';
  rowArr[13] = newId;
...
function deleteExpense_(id) {
  ...
  // LIFO Rule: Find the last populated row index
  ...
  // Verify target is the last recorded entry (LIFO policy)
  var isLastEntry = (
